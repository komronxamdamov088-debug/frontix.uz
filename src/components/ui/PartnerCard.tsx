...
export function PartnerCard({ partner, index = 0 }: { partner: Partner; index?: number }) {
  ...
  return (
    <motion.div
      ...
      aria-label={`${partner.name} — ${t.partners.tapHint}`}
      initial={{ opacity: 0, y: 24 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true, amount: 0.2 }}
      transition={{ duration: 0.6, delay: (index % 3) * 0.08, ease: [0.16, 1, 0.3, 1] }}
      whileHover={{ y: -6 }}
      style={{ rotateX: flipped ? 0 : rotateX, rotateY: flipped ? 0 : rotateY, transformPerspective: 900 }}
      className="group relative flex h-[380px] cursor-pointer flex-col overflow-hidden rounded-3xl border border-ink/10 dark:border-white/10 bg-paper dark:bg-white/[0.02] shadow-soft outline-none transition-shadow duration-300 hover:shadow-[0_24px_60px_-20px_color-mix(in_srgb,var(--color-brand-500)_45%,transparent)] focus-visible:ring-2 focus-visible:ring-brand-500"
    >
      <span
      ...
      >
        ...
        <div className="absolute inset-0 flex flex-col" style={{ backfaceVisibility: "hidden" }}>
          ...
          <div className="relative flex flex-1 items-center overflow-hidden border-t border-ink/10 dark:border-white/10 p-6">
            ...
            <div className="relative">
              ...
              <span className="mt-1 block text-xs text-ink/50 dark:text-white/50">{t.partners.tapHint}</span>
            </div>
          </div>
        </div>
        ...
        {/* Back face */}
        <div
          className="absolute inset-0 flex flex-col justify-between bg-ink p-8 text-paper dark:bg-paper dark:text-ink"
          style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}
        >
          <div>
            <span className="mb-3 block h-[3px] w-6 rounded-full bg-brand-500" />
            <h3 className="text-lg font-semibold tracking-tight">{partner.name}</h3>
            <p className="mt-3 text-sm leading-relaxed text-paper/75 dark:text-ink/70">
          ...
          {partner.website && (
            <a
              ...
              className="inline-flex items-center gap-2 self-start rounded-full bg-brand-500 px-5 py-2.5 text-sm font-medium text-white transition-colors duration-300 hover:bg-brand-600"
            >
