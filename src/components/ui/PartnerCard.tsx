...
export function PartnerCard({ partner, index = 0 }: { partner: Partner; index?: number }) {
  ...
  return (
    <MotionLink
      ref={ref}
      to={localizePath(lang, `/partners/${partner.slug}`)}
      onMouseMove={handleMouseMove}
      onMouseLeave={handleMouseLeave}
      aria-label={`${partner.name} — ${t.partners.more}`}
    ...
    >
      ...
      <div className="relative flex flex-1 items-center overflow-hidden border-t border-ink/10 dark:border-white/10 p-6">
        ...
        <div className="relative min-w-0">
          ...
          <span className="mt-0.5 block truncate text-sm text-ink/55 dark:text-white/55">{partner.tagline[lang]}</span>
          <span className="mt-2 inline-flex items-center gap-1.5 text-xs font-medium text-brand-500">
            {t.partners.more}
